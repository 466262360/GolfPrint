...
public abstract class FFF
{
  ...
  private Common.DATA_TYPE m_expectedReceiveType;
  protected CallbackInfo m_callbackInfo = new CallbackInfo();
...
//  private Vector<Byte> m_imageBuffer = new Vector(1000000, 1000000);
  ...
  private Common.RECEIVESTATE m_receiveState;
  private Common.RECEIVESUBSTATE m_receiveSubState;
...
//  private ReentrantLock m_imageBufferLock = new ReentrantLock();
  ...
  protected Common.ERROR_CODE m_Error = Common.ERROR_CODE.SUCCESS;

  abstract Common.ERROR_CODE openPort();

  abstract Common.ERROR_CODE closePort();

  abstract boolean isPortOpen();

  abstract Common.ERROR_CODE writeData(Vector<Byte> paramVector);
  

  protected abstract Common.ERROR_CODE writeDataImmediately(Vector<Byte> paramVector);

  FFF(DeviceParameters parameters)
  {
    this.m_deviceParameters = parameters;
    this.m_expectedReceiveType = Common.DATA_TYPE.GENERAL;
    ...
    this.m_receiveState = Common.RECEIVESTATE.RSTATE_SINGLE;
    this.m_receiveSubState = Common.RECEIVESUBSTATE.RSUBSTATE_INITSTATE;
  ...
  public Common.ERROR_CODE getError()
  {
    return this.m_Error;
  }
...
//  }
  ...
  protected void parseOutgoingData(Vector<Byte> data)
  {
    ...
    if ((data.size() > 2) && (((Byte)data.get(0)).byteValue() == Common.ASCII_CONTROL_CODE.DLE.getASCIIValue()) && (((Byte)data.get(1)).byteValue() == Common.ASCII_CONTROL_CODE.EOT.getASCIIValue()))
    {
...
//    }
  ...
  protected void afterCallbackAction(Common.DATA_TYPE dataType)
  {
...
//    }
  ...
  protected Common.ERROR_CODE saveData(Vector<Byte> receivedData)
  {
    byte oneByte = 0;
    int dataCounter = 0;
    int dataSize = 0;
    ...
    byte bACK = Common.ASCII_CONTROL_CODE.ACK.getASCIIValue();
    ...
    while (receivedData.size() > 0)
    {
      ...
      case RSTATE_SINGLE:
        ...
        if ((oneByte & 0x93) == 18)
        {
          ...
          this.m_expectedReceiveType = Common.DATA_TYPE.GENERAL;
          this.m_callbackInfo.ReceivedDataType = Common.DATA_TYPE.DEVICESTATUS;
        }
        else if ((oneByte & 0x93) == 16)
        {
          ...
          this.m_receiveState = Common.RECEIVESTATE.RSTATE_ASB;
        }
        else if (oneByte == 83)
        {
          this.m_dataBlockSize = 0;
          this.m_receiveState = Common.RECEIVESTATE.RSTATE_BLOCK_BINARY;
        }
...
//        }
        else
        {
          ...
          this.m_receiveState = Common.RECEIVESTATE.RSTATE_SINGLE;
          this.m_callbackInfo.ReceivedDataType = Common.DATA_TYPE.GENERAL;
        }
      ...
      case RSTATE_ASB:
        ...
        {
          this.m_receiveState = Common.RECEIVESTATE.RSTATE_SINGLE;
          this.m_callbackInfo.ReceivedDataType = Common.DATA_TYPE.ASB;
        }
        break;
...
//          }
          else if (this.m_receiveCounter > 2L)
          {
            ...
            case RSUBSTATE_FILEINFO:
              this.m_callbackInfo.ReceivedDataType = Common.DATA_TYPE.NOTHING;
              ...
              if (!blockEndFlag.booleanValue())
                continue;
              this.m_receiveSubState = Common.RECEIVESUBSTATE.RSUBSTATE_SIZEINFO;
              this.m_receiveState = Common.RECEIVESTATE.RSTATE_SINGLE;
            ...
            case RSUBSTATE_SIZEINFO:
              ...
              if (!blockEndFlag.booleanValue())
                continue;
              this.m_receiveSubState = Common.RECEIVESUBSTATE.RSUBSTATE_IMAGEDATA;
              this.m_receiveState = Common.RECEIVESTATE.RSTATE_SINGLE;
              writeDataImmediately(v_ACK);
      ...
      case RSTATE_BLOCK_BINARY:
        if (this.m_receiveCounter == 1L)
        {
          ...
          if (this.m_identifier == 32)
          {
            ...
            this.m_sizeInfoBlock.clear();
//            this.m_imageDataBlock.clear();
            this.m_receiveSubState = Common.RECEIVESUBSTATE.RSUBSTATE_FILEINFO;
          }
